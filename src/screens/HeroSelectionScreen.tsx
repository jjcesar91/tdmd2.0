...
import { ChevronLeft, Lock, Eye, Skull, Target, Flag, ShieldCheck, Axe, Link, Shovel, Shuffle, Hammer, Moon, Flame, Zap, Gem, Ghost, Leaf } from 'lucide-react';
        ...
        <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="relative z-10 flex items-center justify-between p-4 bg-gradient-to-b from-black/80 to-transparent">
                ...
                <div className="w-full flex justify-center">
                     <h1 className="text-xl font-bold tracking-widest text-[#d4b483]">HEROES</h1>
                </div>
            </div>
            
            {/* Header Context */}
            <div className="relative pb-6 px-4">
                <p className="text-[10px] text-[#8c8c8c] uppercase tracking-[0.3em] font-bold text-center mb-6">Assemble Your Party</p>
                    ...
                    {[0, 1, 2].map(i => {
                        ...
                        return (
                            <div key={i} className={`w-10 h-10 rounded-full border-2 transition-all duration-300 flex items-center justify-center backdrop-blur-sm ${
                                selectedHero 
                                ? 'bg-[#d4b483]/20 border-[#d4b483] shadow-[0_0_15px_rgba(212,180,131,0.3)] scale-110' 
                    ...
                    return (
                        ...
                        <div
                            ...
                            isLocked 
                                ...
                                ? 'ring-2 ring-[#d4b483] shadow-[0_0_20px_rgba(212,180,131,0.2)] scale-[1.02] z-10'
                                : 'hover:shadow-lg hover:shadow-black/60 hover:scale-[1.02] border border-stone-800/50 hover:border-[#d4b483]/50')
                            }`}
                            style={{
                            background: isSelected 
                                ? 'linear-gradient(180deg, rgba(212, 180, 131, 0.1) 0%, rgba(28, 25, 23, 0.9) 100%)' // Gold/Stone gradient
                        ...
                        >
                            ...
                            className={`absolute top-2 right-2 w-7 h-7 rounded-full bg-stone-900/80 border border-stone-600 hover:bg-stone-800 hover:border-[#d4b483] flex items-center justify-center transition-all shadow-lg z-30 ${
                                isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                            }`}
                            >
                            <Eye size={14} className="text-stone-300 group-hover:text-[#d4b483]" />
                            </button>
                            ...
                            <div className="absolute bottom-[20%] left-1/2 -translate-x-1/2 translate-y-1/2 z-20" title={hero.archetype}>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 shadow-lg transition-colors duration-300 ${
                                 isSelected ? 'bg-stone-800 border-[#d4b483] shadow-[#d4b483]/20' : 'bg-stone-900 border-stone-600 shadow-black/50'
                                }`}>
                            ...
                            <div className={`relative h-[20%] flex flex-col items-center justify-center px-1 border-t ${
                                isSelected ? 'border-[#d4b483]/30 bg-stone-900/90' : 'border-stone-800/80 bg-stone-950/80'
                            }`}>
                                <div className={`text-xs font-bold font-serif leading-tight text-center truncate w-full ${
                                isSelected ? 'text-[#d4b483]' : 'text-stone-400 group-hover:text-[#d4b483]/70'
                                }`}>
                                The {hero.name}
                                </div>
                            </div>
            ...
            <div className="relative p-4 bg-gradient-to-t from-black via-black/90 to-transparent pb-8">
                 <button 
                    onClick={onNext}
                 ...
                 >
                    ...
                    <div className={`absolute inset-0 transform skew-x-[-10deg] rounded-sm transition-colors border-2 ${
                        selectedHeroes.length === 3 
                        ? 'bg-[#8c7a59] group-hover:bg-[#9d8a67] border-[#b59e75] shadow-[0_0_20px_rgba(140,122,89,0.3)]' 
                     ...
                     <div className="relative flex items-center justify-center gap-2 py-3 px-6">
                         <span className={`font-black text-xl tracking-widest uppercase ${
                            selectedHeroes.length === 3 ? 'text-stone-900' : 'text-stone-500'
                         }`}>
                             {selectedHeroes.length === 3 ? 'Confirm' : `${selectedHeroes.length}/3 Selected`}
                         </span>
                     </div>
                 </button>
            </div>
